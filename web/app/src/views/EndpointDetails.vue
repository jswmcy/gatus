        ...
        <div v-if="endpointStatus && endpointStatus.name" class="space-y-6">
          <div class="flex items-start justify-between">
            <div>
              ...
              <div class="flex items-center gap-3 text-muted-foreground mt-2">
                <span v-if="endpointStatus.group">{{ t('endpointDetails.group', { group: endpointStatus.group }) }}</span>
          ...
          <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            <Card>
              ...
              <CardContent>
                <div class="text-2xl font-bold">{{ currentHealthStatus === 'healthy' ? t('status.operational') : t('status.issuesDetected') }}</div>
              </CardContent>
            </Card>
