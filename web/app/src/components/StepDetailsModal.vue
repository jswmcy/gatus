      ...
      <div class="p-4 space-y-4 overflow-y-auto max-h-[60vh]">
        ...
        <div v-if="step.isAlwaysRun" class="flex flex-wrap gap-2">
          <div class="flex items-center gap-2 px-3 py-2 bg-blue-50 dark:bg-blue-900/30 rounded-lg border border-blue-200 dark:border-blue-700">
            ...
            <div>
              <p class="text-sm font-medium text-blue-900 dark:text-blue-200">{{ t('stepDetails.alwaysRun') }}</p>
              <p class="text-xs text-blue-600 dark:text-blue-400">{{ t('stepDetails.alwaysRunDesc') }}</p>
            </div>
          </div>
        </div>
        ...
        <div v-if="step.result" class="space-y-2">
          <h3 class="text-sm font-medium flex items-center gap-2">
            ...
            {{ t('stepDetails.response') }}
          </h3>
          <div class="grid grid-cols-2 gap-4 text-xs">
            <div>
              <span class="text-muted-foreground">{{ t('stepDetails.duration') }}:</span>
              <p class="font-mono mt-1">{{ formatDuration(step.result.duration) }}</p>
            </div>
            <div>
              <span class="text-muted-foreground">{{ t('stepDetails.success') }}:</span>
        ...
        <div v-if="step.endpoint" class="space-y-2">
          <h3 class="text-sm font-medium flex items-center gap-2">
            ...
            {{ t('stepDetails.endpointConfiguration') }}
          </h3>
          <div class="space-y-3 text-xs">
            <div v-if="step.endpoint.url">
              <span class="text-muted-foreground">{{ t('stepDetails.url') }}:</span>
              <p class="font-mono mt-1 break-all">{{ step.endpoint.url }}</p>
            </div>
            <div v-if="step.endpoint.method">
              <span class="text-muted-foreground">{{ t('stepDetails.method') }}:</span>
              <p class="mt-1 font-medium">{{ step.endpoint.method }}</p>
            </div>
            <div v-if="step.endpoint.interval">
              <span class="text-muted-foreground">{{ t('stepDetails.interval') }}:</span>
              <p class="mt-1">{{ step.endpoint.interval }}</p>
            </div>
            <div v-if="step.endpoint.timeout">
              <span class="text-muted-foreground">{{ t('stepDetails.timeout') }}:</span>
