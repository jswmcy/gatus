    ...
    <div v-if="result" class="space-y-2">
      ...
      <div v-if="isSuiteResult" class="flex items-center gap-2">
        ...
        <span class="text-xs font-semibold">
          {{ result.success ? t('stepDetails.tooltip.suitePassed') : t('stepDetails.tooltip.suiteFailed') }}
        </span>
      </div>

      <!-- Timestamp -->
      <div>
        <div class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{{ t('stepDetails.tooltip.timestamp') }}</div>
      ...
      <div v-if="isSuiteResult && result.endpointResults">
        <div class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{{ t('home.endpoints') }}</div>
        <div class="font-mono text-xs">
          <span :class="successCount === endpointCount ? 'text-green-500' : 'text-yellow-500'">
            {{ successCount }}/{{ endpointCount }} {{ t('stepDetails.passed') }}
          </span>
        </div>
        <!-- Endpoint breakdown -->
        <div v-if="result.endpointResults.length > 0" class="mt-1 space-y-0.5">
          ...
          <div v-if="result.endpointResults.length > 5" class="text-xs text-muted-foreground">
            ... {{ t('stepDetails.tooltip.andMore', { count: result.endpointResults.length - 5 }) }}
          </div>
        </div>
      </div>
      ...
      <div>
        <div class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
          {{ isSuiteResult ? t('stepDetails.tooltip.totalDuration') : t('stepDetails.tooltip.responseTime') }}
        </div>
        <div class="font-mono text-xs">
      ...
      <div v-if="!isSuiteResult && result.conditionResults && result.conditionResults.length">
        <div class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{{ t('stepDetails.tooltip.conditions') }}</div>
        <div class="font-mono text-xs space-y-0.5">
          ...
          >
            ...
            <span class="break-all">{{ translateCondition(conditionResult.condition) }}</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="result.errors && result.errors.length">
        <div class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{{ t('stepDetails.tooltip.errors') }}</div>
        <div class="font-mono text-xs space-y-0.5">
